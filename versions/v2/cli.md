...
A pre-configured shell script is included at `Assets/MetaGuard/CI/metaguard_scan.sh`. It sets sensible defaults and handles the Unity path and project path as positional arguments.

```bash
# Run with defaults
./Assets/MetaGuard/CI/metaguard_scan.sh /path/to/unity /path/to/project

# Override threshold via environment variable
RISK_THRESHOLD=2 ./Assets/MetaGuard/CI/metaguard_scan.sh /path/to/unity /path/to/project
```
